...
# Run Connect Button Playwright Test in CI/CD Environment
# This script runs a Playwright test to verify the Connect button functionality
...
echo -e "${BLUE}╔═════════════════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║      CONNECT BUTTON PLAYWRIGHT TEST (CI/CD)         ║${NC}"
echo -e "${BLUE}╚═════════════════════════════════════════════════════╝${NC}"
echo ""
...
echo ""
echo -e "Running Playwright Connect Button test..."
mkdir -p test-results
# Set SERVER_URL environment variable for the test
export SERVER_URL="http://localhost:5000"
node tests/playwright/verify_connect_button.js 2>&1 | tee test-results/connect-button-playwright-test.log
...
if [ $TEST_RESULT -eq 0 ]; then
  echo -e "${GREEN}✅ PASSED: Connect Button Playwright Test${NC}"
  exit 0
else
  echo -e "${RED}❌ FAILED: Connect Button Playwright Test${NC}"
  echo -e "${YELLOW}See test-results/connect-button-playwright-test.log for details${NC}"
  exit 1
fi